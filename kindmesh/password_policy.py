...
def validate_password(password: str, min_length: int = 8) -> Tuple[bool, List[str]]:
    ...
    including minimum length, character variety (lowercase, uppercase, digits,
...
def get_password_requirements() -> str:
    ...
    This function returns a formatted string that describes the password
    requirements in a user-friendly way, suitable for display in the UI.
    
    Returns:
        str: Description of password requirements formatted as a markdown list
    """
    return """
    Password must:
